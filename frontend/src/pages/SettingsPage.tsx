...
} from "../components/Desktop/UpdateChecker";
import {
	checkHealth,
	fetchSpeechHealth,
	getInferenceSource,
...
import { type ThemeMode, useAppStore } from "../lib/store";
...
function OllamaModelList() {
	...
	useEffect(() => {
		fetch("http://localhost:11434/api/tags")
			...
			.then((data) =>
				setModels(
					(data.models || []).map((m: { name: string; size: number }) => ({ name: m.name, size: m.size })),
				),
			)
			.catch(() => setModels([]));
	}, []);
...
export function SettingsPage() {
	...
	const [updateCheckState, setUpdateCheckState] = useState<
		"idle" | "checking" | "available" | "latest"
	>("idle");
	...
	const handleCheckNow = useCallback(async () => {
		// biome-ignore lint/suspicious/noExplicitAny: Tauri runtime detection
	...
	const [memoryTopK, setMemoryTopK] = useState(() => {
		try {
			return parseInt(
				localStorage.getItem("openjarvis-memory-top-k") || "5",
				10,
			);
		} catch {
			return 5;
	...
	const saveSource = useCallback(async () => {
		try {
			...
			setSrcMsg(e instanceof Error ? e.message : "Failed to save.");
		}
			...
			<div className="max-w-2xl mx-auto">
				<header className="mb-6">
					...
					<p
						...
						style={{ color: "var(--color-text-secondary)" }}
					>
						App preferences — appearance, model defaults, keyboard shortcuts,
						and data management.
					</p>
				</header>
				...
				<div className="flex flex-col gap-4">
					...
					<Section title="Appearance">
						...
						<SettingRow label="Font size">
							<select
								...
								onChange={(e) => {
									// biome-ignore lint/suspicious/noExplicitAny: fontSize union not exported
					...
					<Section title="Connection">
						...
						<SettingRow
							label="API key"
							description="Required only if the server was started with an API key"
						>
							<input
								type="password"
					...
					<Section title="Inference source">
						<SettingRow
							label="Source"
							description="Where the app runs models. Applies after restart."
						>
							<select
								value={srcKind}
								onChange={(e) => {
									setSrcKind(e.target.value as InferenceSource["kind"]);
									setSrcMsg("");
								}}
								className="text-sm px-3 py-1.5 rounded-lg outline-none w-56"
								style={{
						...
						{srcKind === "custom" && (
							<>
								<SettingRow
									label="Server URL"
									description="e.g. LM Studio: http://localhost:1234/v1"
								>
									<input
										type="text"
										value={customHost}
								...
								<SettingRow
									label="Model"
									description="Model id served by your endpoint"
								>
									<input
										type="text"
										value={customModel}
								...
								<SettingRow
									label="Server type"
									description="OpenAI-compatible engine"
								>
									<select
										value={customEngine}
										onChange={(e) => {
								...
								<SettingRow
									...
									description="Only if your server requires one"
								>
									<input
										type="password"
										value={customKey}
					...
					<Section title="Updates">
						<SettingRow
							label="Auto-update"
							description="Check for new desktop builds automatically every 30 minutes"
						>
							<button
								type="button"
						...
						<SettingRow
							...
							description="Manually check for a new version right now"
						>
							<button
								type="button"
							...
							>
								<RefreshCw
									size={12}
									className={
										updateCheckState === "checking" ? "animate-spin" : ""
									}
								/>
