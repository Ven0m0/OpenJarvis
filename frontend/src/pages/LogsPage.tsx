...
export function LogsPage() {
	...
	const handleCopy = async () => {
		const text = logEntries
			.map(
				(e) =>
					`${formatTime(e.timestamp)} [${e.level}] [${e.category}] ${e.message}`,
			)
			.join("\n");
		...
		<div className="flex-1 flex flex-col overflow-hidden px-6 py-10">
			<div className="max-w-4xl mx-auto w-full flex flex-col flex-1 overflow-hidden">
				<header className="mb-6 shrink-0">
					...
					>
						Recent activity — chat events, model switches, tool calls, and
						system messages from this session.
					</p>
				</header>
				...
				>
					{logEntries.length === 0 ? (
						<div
							...
							style={{ color: "var(--color-text-tertiary)" }}
						>
							No log entries yet. Logs appear as you chat, switch models, and
							interact with the app.
						</div>
					) : (
						logEntries.map((entry, i) => (
							// biome-ignore lint/suspicious/noArrayIndexKey: log entries have no stable unique id
