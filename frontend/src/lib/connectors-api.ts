...
export async function getConnector(id: string): Promise<ConnectorInfo> {
	const res = await fetch(
		`${getBase()}/v1/connectors/${encodeURIComponent(id)}`,
	);
...
export async function connectSource(
	...
	const res = await fetch(
		`${getBase()}/v1/connectors/${encodeURIComponent(id)}/connect`,
		{
			method: "POST",
...
export function startServerOAuth(
	id: string,
	oauthStartPath?: string,
): Promise<void> {
	const path =
		oauthStartPath || `/v1/connectors/${encodeURIComponent(id)}/oauth/start`;
...
export async function disconnectSource(id: string): Promise<void> {
	const res = await fetch(
		`${getBase()}/v1/connectors/${encodeURIComponent(id)}/disconnect`,
		{
			method: "POST",
		},
	);
...
export async function getSyncStatus(id: string): Promise<SyncStatus> {
	const res = await fetch(
		`${getBase()}/v1/connectors/${encodeURIComponent(id)}/sync`,
	);
	if (!res.ok)
		throw new Error(`Failed to get sync status for ${id}: ${res.status}`);
	return res.json();
}

export async function triggerSync(
	id: string,
): Promise<{ connector_id: string; chunks_indexed: number; status: string }> {
	const res = await fetch(
		`${getBase()}/v1/connectors/${encodeURIComponent(id)}/sync`,
		{
			method: "POST",
