...
const CITATION_RE = /\[(\s*\d+(?:\s*,\s*\d+)*\s*)\](\s+(?=[.,;:!?]))?/g;
...
export function rehypeCitations(options: {
	...
	function transformText(text: string): HastNode[] | null {
		...
		while (m !== null) {
			...
			const allMatched =
				resolved.length > 0 && resolved.every((r) => r.src?.url);
			...
			if (allMatched) {
				for (const r of resolved) {
					// biome-ignore lint/style/noNonNullAssertion: allMatched guarantees src is defined
