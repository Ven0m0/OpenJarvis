...
export function XRayFooter({ usage, telemetry, isResearch = false }: Props) {
	...
	const summary =
		parts.length > 0 ? parts.join(" - ") : `${usage?.total_tokens} tokens`;
	...
	if (isResearch) {
		...
		rows.push({
			label: "Engine",
			value: `${telemetry.engine}${modelDetail ? ` (${modelDetail})` : ""}`,
		});
	}
	if (usage) {
		const tokenParts = [
	...
	if (telemetry?.suggested_max_tokens) {
		rows.push({
			label: "Token budget",
			value: `${telemetry.suggested_max_tokens}`,
		});
	}
	if (telemetry?.tokens_per_sec) {
		rows.push({
			label: "Speed",
			value: `${Math.round(telemetry.tokens_per_sec)} tok/s`,
		});
	}
	if (telemetry?.ttft_ms != null || telemetry?.total_ms != null) {
		const latencyParts: string[] = [];
		if (telemetry.ttft_ms != null)
			latencyParts.push(`TTFT ${formatMs(telemetry.ttft_ms)}`);
		if (telemetry.total_ms != null)
			latencyParts.push(`Total ${formatMs(telemetry.total_ms)}`);
