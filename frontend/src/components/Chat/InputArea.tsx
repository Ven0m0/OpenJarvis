...
import { getSyncStatus, listConnectors } from "../../lib/connectors-api";
...
export function InputArea() {
	...
	const _messages = useAppStore((s) => s.messages);
					...
					if (ev.type === "search_call") {
						...
						if (ev.sources) {
							for (const src of ev.sources) {
								if (
									src &&
									typeof src.ref === "number" &&
									!researchSourcesByRef.has(src.ref)
								) {
			...
			<div className="mb-2 flex flex-col gap-1">
				...
				{deepResearch && corpusSync.syncing && corpusSync.itemsSynced > 0 && (
					...
					>
						Searching over{" "}
						<span
							key={corpusSync.itemsSynced}
							className="sync-bump"
							style={{ color: "var(--color-text-secondary)" }}
						>
							{corpusSync.itemsSynced.toLocaleString()}
						</span>{" "}
						items — sync in progress, results will improve as more data is
						indexed.
					</div>
				)}
			</div>
