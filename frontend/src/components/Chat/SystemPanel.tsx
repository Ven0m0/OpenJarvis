...
const CLOUD_PRICING = [
	{ name: "GPT-5.3", input: 2.0, output: 10.0, primary: true },
	{ name: "Claude Opus 4.6", input: 5.0, output: 25.0, primary: false },
	{ name: "Gemini 3.1 Pro", input: 2.0, output: 12.0, primary: false },
];
	...
	const fetchData = useCallback(async () => {
		try {
			...
			const [energyRes, telRes] = await Promise.allSettled([
				fetch(`${base}/v1/telemetry/energy`).then((r) =>
					r.ok ? r.json() : null,
				),
				fetch(`${base}/v1/telemetry/stats`).then((r) =>
					r.ok ? r.json() : null,
				),
			]);
				...
				<section>
					...
					<div className="grid grid-cols-2 gap-2">
						<MiniStat
							...
							label="Requests"
							value={String(
								savings?.total_calls ?? telemetry?.total_requests ?? 0,
							)}
						/>
						<MiniStat
							icon={Hash}
							label="Output Tokens"
							value={formatNumber(
								savings?.total_completion_tokens ??
				...
				<section>
					...
					<div className="grid grid-cols-2 gap-2">
						...
						<MiniStat
							icon={Zap}
							label="Power"
							value={(liveEnergy?.power_w ?? energy?.avg_power_w ?? 0).toFixed(
								1,
							)}
							unit="W"
						/>
