...
// biome-ignore lint/suspicious/noExplicitAny: rehype component props have no stable public type
...
export function MessageBubble({ message, isLive = false }: Props) {
	...
	const sourcesMap = useMemo(() => {
		const m = new Map<
			number,
			NonNullable<ChatMessage["researchSources"]>[number]
		>();
	...
	const rehypePlugins = useMemo(() => {
		// biome-ignore lint/suspicious/noExplicitAny: rehype plugin list has no stable public type
		const base: any[] = [[rehypeHighlight, { detect: true }], rehypeKatex];
		if (sourcesMap.size > 0)
			base.push([rehypeCitations, { sources: sourcesMap }]);
		return base;
	}, [sourcesMap]);
	...
	return (
		<div className="group mb-6">
			...
			{/* Spoken reply (text-to-speech of the assistant's message) */}
			{message.audio?.url && <AudioPlayer src={message.audio.url} autoPlay />}
