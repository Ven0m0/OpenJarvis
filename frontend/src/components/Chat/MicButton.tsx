...
export function MicButton({
	...
	return (
		// biome-ignore lint/a11y/noStaticElementInteractions: tooltip wrapper only uses hover events, no click
		<div
			className="relative"
		...
		>
			<button
				...
				style={{
					background:
						state === "recording" ? "var(--color-error)" : "transparent",
					color:
						state === "recording"
							? "white"
							: isInactive
					...
					animation:
						state === "recording" ? "pulse 1.5s ease-in-out infinite" : "none",
				}}
			>
				{state === "transcribing" ? (
