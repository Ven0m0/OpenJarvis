...
const CATALOGUE_MODELS = [
	{
		id: "qwen3.5:0.8b",
		size: "~1 GB",
		desc: "Qwen 3.5 0.8B — fast, lightweight",
	},
	{ id: "qwen3.5:2b", size: "~2.7 GB", desc: "Qwen 3.5 2B" },
	{
		id: "qwen3.5:4b",
		size: "~3.4 GB",
		desc: "Qwen 3.5 4B — recommended default",
	},
	...
	},
	{
		...
		models: [
			{ id: "openrouter/auto", desc: "Auto — best model for the task" },
			{
				...
				desc: "Claude Sonnet 4 via OpenRouter",
			},
			{
				id: "openrouter/deepseek/deepseek-r1",
				desc: "DeepSeek R1 via OpenRouter",
			},
		],
	},
];
...
export function CommandPalette() {
	...
	const [apiKeys, setApiKeys] = useState<Record<string, string>>(() => {
		...
		for (const p of CLOUD_PROVIDERS)
			keys[p.storageKey] = getStoredKey(p.storageKey);
		return keys;
	});
	...
	const handleSelect = async (modelId: string) => {
		...
		if (modelId !== previousModel) {
			const { createConversation, setModelLoading, addLogEntry } =
			...
			try {
				...
				addLogEntry({
					...
					level: "error",
					category: "model",
					message: `Failed to load ${modelId}: ${e instanceof Error ? e.message : String(e)}`,
				});
			} finally {
	...
	const handleSaveKey = async (provider: CloudProvider, value: string) => {
		...
		if (isTauri()) {
			try {
				...
				await invoke("save_cloud_key", {
					keyName: provider.envKey,
					keyValue: value,
				});
			} catch {}
		}
	...
	const handleKeyDown = (e: React.KeyboardEvent) => {
		if (e.key === "Escape") {
			...
			e.preventDefault();
			// biome-ignore lint/suspicious/noExplicitAny: filtered is a union type; .id exists on all variants
	...
	return (
		...
		// biome-ignore lint/a11y/useKeyWithClickEvents: Escape key handled by keydown listener
		<div
		...
		>
			...
			{/* biome-ignore lint/a11y/noStaticElementInteractions: stops propagation to backdrop */}
			...
			>
					...
					{tab === "installed" ? (
						filtered.length === 0 ? (
							...
							(filtered as typeof models).map((model, idx) => {
								...
								return (
									// biome-ignore lint/a11y/noStaticElementInteractions: hover-only highlight, no click on wrapper
									<div
										key={model.id}
						...
						<div className="px-4 py-2">
							<div
								...
								style={{ color: "var(--color-text-tertiary)" }}
							>
								Add your API keys to use cloud models. Keys are stored locally
								on your device only.
							</div>
											...
											>
												...
												<input
													...
													onChange={(e) =>
														setApiKeys((prev) => ({
															...prev,
															[provider.storageKey]: e.target.value,
														}))
													}
													onBlur={() =>
														handleSaveKey(
															provider,
															apiKeys[provider.storageKey] || "",
														)
													}
												...
												<button
													...
													onClick={() =>
														setShowKeys((prev) => ({
															...prev,
															[provider.storageKey]: !prev[provider.storageKey],
														}))
													}
				...
				>
					{tab === "installed" ? (
						...
						<span>
							API keys are stored locally and never sent to OpenJarvis servers
						</span>
					)}
				</div>
			</div>
