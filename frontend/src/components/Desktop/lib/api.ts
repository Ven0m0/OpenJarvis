...
export async function fetchManagedAgents(
	...
	const data = await request<{ agents: ManagedAgent[] }>(
		apiUrl,
		"/v1/managed-agents",
	);
	return data.agents || [];
...
export async function fetchAgentTasks(
	...
	const data = await request<{ tasks: AgentTask[] }>(
		apiUrl,
...
export async function pauseManagedAgent(
	apiUrl: string,
	agentId: string,
): Promise<void> {
...
export async function resumeManagedAgent(
	apiUrl: string,
	agentId: string,
): Promise<void> {
...
export async function recoverManagedAgent(
	...
	return request<unknown>(apiUrl, `/v1/managed-agents/${agentId}/recover`, {
		method: "POST",
	});
}
...
export async function fetchLearningLog(
	...
	const data = await request<{ learning_log: LearningLogEntry[] }>(
		apiUrl,
...
export async function fetchAgentTraces(
	...
	const data = await request<{ traces: AgentTrace[] }>(
		apiUrl,
...
export async function fetchAgentTrace(
	...
	return request<AgentTraceDetail>(
		apiUrl,
		`/v1/managed-agents/${agentId}/traces/${traceId}`,
	);
}
