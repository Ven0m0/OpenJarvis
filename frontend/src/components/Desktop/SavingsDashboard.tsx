...
export function SavingsDashboard({ apiUrl }: { apiUrl: string }) {
	...
	const [displayName, setDisplayName] = useState(
		localStorage.getItem(OPTIN_NAME_KEY) || "",
	);
	const [emailStored, setEmailStored] = useState(
		localStorage.getItem(OPTIN_EMAIL_KEY) || "",
	);
      ...
      `}</style>
			...
			{showOptIn ? (
				<div style={{ ...styles.statCard, marginBottom: 24, padding: 20 }}>
					...
					>
						Opt in to privately share your savings for the chance to win a Mac
						Mini!
					</div>
					<div style={{ marginBottom: 8 }}>
						<input
							...
							onKeyDown={(e) => {
								if (e.key === "Enter") handleOptInJoin();
							}}
			...
			>
				*Savings estimates assume local models (e.g. Qwen, Nemotron, Kimi)
				produce roughly the same number of tokens per request, on average, as
				closed-source cloud models.
			</div>
			...
			{cloudAgent && (
				<>
					...
					<div style={styles.cloudAgentCard}>
						...
						<div style={styles.providerModel}>
							based on published API pricing tiers
						</div>
