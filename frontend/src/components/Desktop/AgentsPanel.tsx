			...
			<div style={dialogStyle}>
							...
							<div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
								<button
									...
									style={{
										...
										borderRadius: 8,
										background:
											wizard.templateId === "" ? `${C.accent}20` : C.surface0,
								...
								>
									...
									<div
										style={{ color: C.overlay0, fontSize: 11, marginTop: 2 }}
									>
									...
									{AVAILABLE_TOOLS.map((tool) => (
										<label
											...
											style={{
												...
												borderRadius: 6,
												cursor: "pointer",
												background: wizard.selectedTools.includes(tool.id)
													? `${C.accent}15`
													: C.surface0,
								...
								<div style={{ display: "flex", alignItems: "flex-end" }}>
									...
									>
										<input
											...
											onChange={(e) =>
												update({ learningEnabled: e.target.checked })
											}
										/>
										<span style={{ color: C.text, fontSize: 12 }}>
											Enable Learning
							...
							>
								{[
									...
									[
										"Template",
										wizard.templateId
											? (templates.find((t) => t.id === wizard.templateId)
													?.name ?? wizard.templateId)
											: "Custom",
									],
									[
										"Schedule",
				...
				>
					<button
						...
						onClick={() =>
							wizard.step > 1
								? update({ step: (wizard.step - 1) as 1 | 2 | 3 })
								: onClose()
						}
						style={{
							padding: "8px 16px",
...
function InteractTab({ apiUrl, agentId }: { apiUrl: string; agentId: string }) {
	...
	return (
		...
		>
			...
			{/* Error */}
			{error && (
				<div style={{ color: C.red, fontSize: 12, padding: "4px 0" }}>
					{error}
				</div>
			)}
		...
		<div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
			{tasks.map((t) => (
				...
				>
					...
					>
						<span
							style={{ color: C.text, fontSize: 13, flex: 1, marginRight: 12 }}
						>
							{t.description}
						</span>
						<span
		...
		<div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
			...
			>
				...
				<div style={{ ...rowStyle, borderBottom: "none" }}>
					...
					<span style={valueStyle}>
						{agent.budget !== undefined ? `$${agent.budget}` : "Unlimited"}
					</span>
				</div>
				{/* Budget progress bar */}
...
function LearningTabContent({
	...
	useEffect(() => {
		fetchLearningLog(apiUrl, agentId)
			.then(setLogs)
			.catch(() => {});
	}, [apiUrl, agentId]);
...
function LogsTabContent({
	...
	useEffect(() => {
		fetchAgentTraces(apiUrl, agentId)
			.then(setTraces)
			.catch(() => {});
	}, [apiUrl, agentId]);
		...
		<div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
			...
			<div style={{ marginBottom: 16 }}>
				...
				>
					...
					/>
					<h2
						style={{ color: C.text, fontSize: 18, fontWeight: 700, margin: 0 }}
					>
						{agent.name}
					</h2>
				</div>
				<div style={{ color: C.overlay0, fontSize: 12 }}>
					{agent.agent_type} · Schedule:{" "}
					{formatSchedule(agent.schedule_type, agent.schedule_value)} · Last
					run: {formatRelativeTime(agent.last_run_at)}
				</div>
			</div>
			...
			<div style={{ flex: 1, overflowY: "auto", minHeight: 0 }}>
				...
				{activeTab === "interact" && (
					<InteractTab apiUrl={apiUrl} agentId={agent.id} />
				)}
				...
				{activeTab === "learning" && (
					<LearningTabContent
						apiUrl={apiUrl}
						agentId={agent.id}
						learningEnabled={!!agent.learning_enabled}
					/>
				)}
				{activeTab === "logs" && (
...
export function AgentsPanel({ apiUrl }: Props) {
	...
	useEffect(() => {
		if (selectedId) {
			fetchAgentTasks(apiUrl, selectedId)
				.then(setTasks)
				.catch(() => setTasks([]));
		}
	}, [apiUrl, selectedId]);
			...
			>
				...
				<div style={{ flex: 1, overflowY: "auto", padding: "8px 10px" }}>
					{agents.map((a) => {
						...
						return (
							// biome-ignore lint/a11y/noStaticElementInteractions: agent list item with complex layout
