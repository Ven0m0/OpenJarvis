...
import type { ConnectorMeta, ConnectRequest } from "../../types/connectors";
...
function LocalPanel({
	...
	return (
		<div className="flex flex-col gap-4">
			<p className="text-sm" style={{ color: "var(--color-text-secondary)" }}>
				{displayName} reads data directly from your Mac. Make sure the app is
				installed and Full Disk Access is granted to OpenJarvis in System
				Settings.
			</p>
			<div
			...
			>
				<strong>System Settings</strong> → Privacy &amp; Security → Full Disk
				Access → enable OpenJarvis
			</div>
			<button
				type="button"
				onClick={() => onConnect({})}
			...
			<div style={{ display: "flex", gap: 8 }}>
				<button
					...
					style={{
						...
						padding: 10,
						background:
							isConnecting || (fields.length > 0 && !allFilled)
...
export function SourceConnectFlow({
	...
	const advanceToNext = (currentIndex: number) => {
		const next = entries.findIndex(
			(e, i) => i > currentIndex && e.state === "pending",
		);
		if (next !== -1) {
			setActiveIndex(next);
		} else {
	...
	const allDone = entries.every(
		(e) => e.state === "connected" || e.state === "skipped",
	);
